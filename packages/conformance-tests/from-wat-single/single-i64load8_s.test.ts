...
import { entry } from './single-i64load8_s.actual';
...
test.each(t)('i64load8_s($a) === $load8_s (%#)', async ({ a, a_binary64, load8_s, load8_s_binary64 }) => {
  expect(a).toEqual(twosComplementToBigInt(a_binary64));
  expect(a_binary64).toEqual(bigIntToTwosComplement(a));
  expect(load8_s).toEqual(twosComplementToBigInt(load8_s_binary64));
  expect(load8_s_binary64).toEqual(bigIntToTwosComplement(load8_s));
  
  const entry = await getWasm<Ops>("from-wat-single", 'single-i64load8_s');
