...
import { entry } from './single-i64load16_u.actual';
...
test.each(t)('i64load16_u($a) === $load16_u (%#)', async ({ a, a_binary64, load16_u, load16_u_binary64 }) => {
  expect(a).toEqual(twosComplementToBigInt(a_binary64));
  expect(a_binary64).toEqual(bigIntToTwosComplement(a));
  expect(load16_u).toEqual(twosComplementToBigInt(load16_u_binary64));
  expect(load16_u_binary64).toEqual(bigIntToTwosComplement(load16_u));
  
  const entry = await getWasm<Ops>("from-wat-single", 'single-i64load16_u');
