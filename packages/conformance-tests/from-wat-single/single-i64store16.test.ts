...
import { entry } from './single-i64store16.actual';
...
test.each(t)('i64store16($a) === $store16 (%#)', async ({ a, a_binary64, store16, store16_binary64 }) => {
  expect(a).toEqual(twosComplementToBigInt(a_binary64));
  expect(a_binary64).toEqual(bigIntToTwosComplement(a));
  expect(store16).toEqual(twosComplementToBigInt(store16_binary64));
  expect(store16_binary64).toEqual(bigIntToTwosComplement(store16));
  
  const entry = await getWasm<Ops>("from-wat-single", 'single-i64store16');
