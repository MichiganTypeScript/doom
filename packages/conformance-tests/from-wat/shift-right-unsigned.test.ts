...
import type { entry } from './shift-right-unsigned.actual.d.ts'
import { shiftRightUnsignedTests } from '../../ts-type-math/test-cases/shift-right.ts';

import { getWasm } from '../utils.ts'
import { expect, test } from 'vitest';

const name = 'shift-right-unsigned';
test(name, async () => {
  const entry = await getWasm('from-wat', name);
  expect(shiftRightUnsignedTests).toHaveLength(28);
  shiftRightUnsignedTests.forEach(({ a, b, expected }) => {
    expect(entry(a, b) >>> 0).toStrictEqual(expected);
  });
});

type S = typeof shiftRightUnsignedTests;
type i = 2
type a = S[i]['a']        // =>
type b = S[i]['b']        // =>
type t = entry<[a, b]>    // =>
