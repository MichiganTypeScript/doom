export type StringToTSNumber<T extends string> =
  T extends `${infer N extends number}`
  ? N
  : never;

export type StringToTSBigInt<T extends string> =
  T extends `${infer N extends bigint}`
  ? N
  : never;

export type TsNumberToString<T extends number> = `${T}`;
export type TsBigIntToString<T extends bigint> = `${T}`;
