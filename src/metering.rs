...
use wasmtime::{Config, Engine, Instance, Module, Result, Store, Trap};
...
pub fn meter() -> Result<()> {
    ...
    let module = Module::from_file(store.engine(), "src/fuel.wat")?;
    let instance = Instance::new(&mut store, &module, &[])?;

    // Invoke `fibonacci` export with higher and higher numbers until we exhaust our fuel.
    let fibonacci = instance.get_typed_func::<i32, i32>(&mut store, "entry")?;
    for n in 1.. {
