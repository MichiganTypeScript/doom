import { Func, runProgram } from '../../program.ts'

type $__wasm_call_ctors<
  RESULT extends Func = {
    kind: 'func';
    params: [];
    result: number;
    locals: [];
    instructions: [
      { kind: 'Call'; id: '$emscripten_stack_init' },
    ];
  }
> = RESULT

type $countAliveNeighbors<
  RESULT extends Func = {
    kind: 'func';
    params: ['$p0', '$p1', '$p2', '$p3'];
    result: number;
    locals: ['$l4', '$l5', '$l6', '$l7', '$l8', '$l9', '$l10', '$l11', '$l12', '$l13', '$l14', '$l15', '$l16', '$l17', '$l18', '$l19', '$l20', '$l21', '$l22', '$l23', '$l24', '$l25', '$l26', '$l27', '$l28', '$l29', '$l30', '$l31', '$l32', '$l33', '$l34', '$l35', '$l36', '$l37', '$l38', '$l39', '$l40', '$l41', '$l42', '$l43', '$l44', '$l45', '$l46', '$l47', '$l48', '$l49', '$l50', '$l51', '$l52', '$l53', '$l54', '$l55', '$l56', '$l57', '$l58', '$l59', '$l60', '$l61', '$l62', '$l63', '$l64', '$l65', '$l66', '$l67', '$l68', '$l69', '$l70', '$l71', '$l72', '$l73', '$l74', '$l75', '$l76', '$l77', '$l78', '$l79', '$l80', '$l81'];
    instructions: [
      { kind: 'GlobalGet'; id: '$__stack_pointer' },
      { kind: 'LocalSet'; id: '$l4' },
      { kind: 'Const'; value: 32 },
      { kind: 'LocalSet'; id: '$l5' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l5' },
      { kind: 'Subtract' },
      { kind: 'LocalSet'; id: '$l6' },
      { kind: 'LocalGet'; id: '$l6' },
      { kind: 'LocalGet'; id: '$p1' },
      { kind: 'I32Store'; offset: 28 },
      { kind: 'LocalGet'; id: '$l6' },
      { kind: 'LocalGet'; id: '$p2' },
      { kind: 'I32Store'; offset: 24 },
      { kind: 'LocalGet'; id: '$l6' },
      { kind: 'LocalGet'; id: '$p3' },
      { kind: 'I32Store'; offset: 20 },
      { kind: 'LocalGet'; id: '$l6' },
      { kind: 'I32Load'; offset: 20 },
      { kind: 'LocalSet'; id: '$l7' },
      { kind: 'Const'; value: 0 },
      { kind: 'LocalSet'; id: '$l8' },
      { kind: 'LocalGet'; id: '$l7' },
      { kind: 'LocalGet'; id: '$l8' },
      { kind: 'I32Store'; offset: 0 },
      { kind: 'Const'; value: -1 },
      { kind: 'LocalSet'; id: '$l9' },
      { kind: 'LocalGet'; id: '$l6' },
      { kind: 'LocalGet'; id: '$l9' },
      { kind: 'I32Store'; offset: 16 },
      { kind: 'Block';
        id: '$B0';
        instructions: [
          { kind: 'Loop';
            id: '$L1';
            instructions: [
              { kind: 'LocalGet'; id: '$l6' },
              { kind: 'I32Load'; offset: 16 },
              { kind: 'LocalSet'; id: '$l10' },
              { kind: 'Const'; value: 1 },
              { kind: 'LocalSet'; id: '$l11' },
              { kind: 'LocalGet'; id: '$l10' },
              { kind: 'LocalSet'; id: '$l12' },
              { kind: 'LocalGet'; id: '$l11' },
              { kind: 'LocalSet'; id: '$l13' },
              { kind: 'LocalGet'; id: '$l12' },
              { kind: 'LocalGet'; id: '$l13' },
              { kind: 'LessThanOrEqual' },
              { kind: 'LocalSet'; id: '$l14' },
              { kind: 'Const'; value: 1 },
              { kind: 'LocalSet'; id: '$l15' },
              { kind: 'LocalGet'; id: '$l14' },
              { kind: 'LocalGet'; id: '$l15' },
              { kind: 'And' },
              { kind: 'LocalSet'; id: '$l16' },
              { kind: 'LocalGet'; id: '$l16' },
              { kind: 'EqualsZero' },
              { kind: 'BranchIf'; id: '$B0' },
              { kind: 'Const'; value: -1 },
              { kind: 'LocalSet'; id: '$l17' },
              { kind: 'LocalGet'; id: '$l6' },
              { kind: 'LocalGet'; id: '$l17' },
              { kind: 'I32Store'; offset: 12 },
              { kind: 'Block';
                id: '$B2';
                instructions: [
                  { kind: 'Loop';
                    id: '$L3';
                    instructions: [
                      { kind: 'LocalGet'; id: '$l6' },
                      { kind: 'I32Load'; offset: 12 },
                      { kind: 'LocalSet'; id: '$l18' },
                      { kind: 'Const'; value: 1 },
                      { kind: 'LocalSet'; id: '$l19' },
                      { kind: 'LocalGet'; id: '$l18' },
                      { kind: 'LocalSet'; id: '$l20' },
                      { kind: 'LocalGet'; id: '$l19' },
                      { kind: 'LocalSet'; id: '$l21' },
                      { kind: 'LocalGet'; id: '$l20' },
                      { kind: 'LocalGet'; id: '$l21' },
                      { kind: 'LessThanOrEqual' },
                      { kind: 'LocalSet'; id: '$l22' },
                      { kind: 'Const'; value: 1 },
                      { kind: 'LocalSet'; id: '$l23' },
                      { kind: 'LocalGet'; id: '$l22' },
                      { kind: 'LocalGet'; id: '$l23' },
                      { kind: 'And' },
                      { kind: 'LocalSet'; id: '$l24' },
                      { kind: 'LocalGet'; id: '$l24' },
                      { kind: 'EqualsZero' },
                      { kind: 'BranchIf'; id: '$B2' },
                      { kind: 'LocalGet'; id: '$l6' },
                      { kind: 'I32Load'; offset: 16 },
                      { kind: 'LocalSet'; id: '$l25' },
                      { kind: 'Block';
                        id: '$B4';
                        instructions: [
                          { kind: 'Block';
                            id: '$B5';
                            instructions: [
                              { kind: 'LocalGet'; id: '$l25' },
                              { kind: 'BranchIf'; id: '$B5' },
                              { kind: 'LocalGet'; id: '$l6' },
                              { kind: 'I32Load'; offset: 12 },
                              { kind: 'LocalSet'; id: '$l26' },
                              { kind: 'LocalGet'; id: '$l26' },
                              { kind: 'BranchIf'; id: '$B5' },
                              { kind: 'Branch'; id: '$B4' },
                            ];
                          },
                          { kind: 'LocalGet'; id: '$l6' },
                          { kind: 'I32Load'; offset: 28 },
                          { kind: 'LocalSet'; id: '$l27' },
                          { kind: 'LocalGet'; id: '$l6' },
                          { kind: 'I32Load'; offset: 16 },
                          { kind: 'LocalSet'; id: '$l28' },
                          { kind: 'LocalGet'; id: '$l27' },
                          { kind: 'LocalGet'; id: '$l28' },
                          { kind: 'Add' },
                          { kind: 'LocalSet'; id: '$l29' },
                          { kind: 'LocalGet'; id: '$l6' },
                          { kind: 'LocalGet'; id: '$l29' },
                          { kind: 'I32Store'; offset: 8 },
                          { kind: 'LocalGet'; id: '$l6' },
                          { kind: 'I32Load'; offset: 24 },
                          { kind: 'LocalSet'; id: '$l30' },
                          { kind: 'LocalGet'; id: '$l6' },
                          { kind: 'I32Load'; offset: 12 },
                          { kind: 'LocalSet'; id: '$l31' },
                          { kind: 'LocalGet'; id: '$l30' },
                          { kind: 'LocalGet'; id: '$l31' },
                          { kind: 'Add' },
                          { kind: 'LocalSet'; id: '$l32' },
                          { kind: 'LocalGet'; id: '$l6' },
                          { kind: 'LocalGet'; id: '$l32' },
                          { kind: 'I32Store'; offset: 4 },
                          { kind: 'LocalGet'; id: '$l6' },
                          { kind: 'I32Load'; offset: 8 },
                          { kind: 'LocalSet'; id: '$l33' },
                          { kind: 'Const'; value: 0 },
                          { kind: 'LocalSet'; id: '$l34' },
                          { kind: 'LocalGet'; id: '$l33' },
                          { kind: 'LocalSet'; id: '$l35' },
                          { kind: 'LocalGet'; id: '$l34' },
                          { kind: 'LocalSet'; id: '$l36' },
                          { kind: 'LocalGet'; id: '$l35' },
                          { kind: 'LocalGet'; id: '$l36' },
                          { kind: 'GreaterThanOrEqual' },
                          { kind: 'LocalSet'; id: '$l37' },
                          { kind: 'Const'; value: 1 },
                          { kind: 'LocalSet'; id: '$l38' },
                          { kind: 'LocalGet'; id: '$l37' },
                          { kind: 'LocalGet'; id: '$l38' },
                          { kind: 'And' },
                          { kind: 'LocalSet'; id: '$l39' },
                          { kind: 'Block';
                            id: '$B6';
                            instructions: [
                              { kind: 'LocalGet'; id: '$l39' },
                              { kind: 'EqualsZero' },
                              { kind: 'BranchIf'; id: '$B6' },
                              { kind: 'LocalGet'; id: '$l6' },
                              { kind: 'I32Load'; offset: 8 },
                              { kind: 'LocalSet'; id: '$l40' },
                              { kind: 'LocalGet'; id: '$p0' },
                              { kind: 'I32Load'; offset: 0 },
                              { kind: 'LocalSet'; id: '$l41' },
                              { kind: 'LocalGet'; id: '$l40' },
                              { kind: 'LocalSet'; id: '$l42' },
                              { kind: 'LocalGet'; id: '$l41' },
                              { kind: 'LocalSet'; id: '$l43' },
                              { kind: 'LocalGet'; id: '$l42' },
                              { kind: 'LocalGet'; id: '$l43' },
                              { kind: 'LessThan' },
                              { kind: 'LocalSet'; id: '$l44' },
                              { kind: 'Const'; value: 1 },
                              { kind: 'LocalSet'; id: '$l45' },
                              { kind: 'LocalGet'; id: '$l44' },
                              { kind: 'LocalGet'; id: '$l45' },
                              { kind: 'And' },
                              { kind: 'LocalSet'; id: '$l46' },
                              { kind: 'LocalGet'; id: '$l46' },
                              { kind: 'EqualsZero' },
                              { kind: 'BranchIf'; id: '$B6' },
                              { kind: 'LocalGet'; id: '$l6' },
                              { kind: 'I32Load'; offset: 4 },
                              { kind: 'LocalSet'; id: '$l47' },
                              { kind: 'Const'; value: 0 },
                              { kind: 'LocalSet'; id: '$l48' },
                              { kind: 'LocalGet'; id: '$l47' },
                              { kind: 'LocalSet'; id: '$l49' },
                              { kind: 'LocalGet'; id: '$l48' },
                              { kind: 'LocalSet'; id: '$l50' },
                              { kind: 'LocalGet'; id: '$l49' },
                              { kind: 'LocalGet'; id: '$l50' },
                              { kind: 'GreaterThanOrEqual' },
                              { kind: 'LocalSet'; id: '$l51' },
                              { kind: 'Const'; value: 1 },
                              { kind: 'LocalSet'; id: '$l52' },
                              { kind: 'LocalGet'; id: '$l51' },
                              { kind: 'LocalGet'; id: '$l52' },
                              { kind: 'And' },
                              { kind: 'LocalSet'; id: '$l53' },
                              { kind: 'LocalGet'; id: '$l53' },
                              { kind: 'EqualsZero' },
                              { kind: 'BranchIf'; id: '$B6' },
                              { kind: 'LocalGet'; id: '$l6' },
                              { kind: 'I32Load'; offset: 4 },
                              { kind: 'LocalSet'; id: '$l54' },
                              { kind: 'LocalGet'; id: '$p0' },
                              { kind: 'I32Load'; offset: 4 },
                              { kind: 'LocalSet'; id: '$l55' },
                              { kind: 'LocalGet'; id: '$l54' },
                              { kind: 'LocalSet'; id: '$l56' },
                              { kind: 'LocalGet'; id: '$l55' },
                              { kind: 'LocalSet'; id: '$l57' },
                              { kind: 'LocalGet'; id: '$l56' },
                              { kind: 'LocalGet'; id: '$l57' },
                              { kind: 'LessThan' },
                              { kind: 'LocalSet'; id: '$l58' },
                              { kind: 'Const'; value: 1 },
                              { kind: 'LocalSet'; id: '$l59' },
                              { kind: 'LocalGet'; id: '$l58' },
                              { kind: 'LocalGet'; id: '$l59' },
                              { kind: 'And' },
                              { kind: 'LocalSet'; id: '$l60' },
                              { kind: 'LocalGet'; id: '$l60' },
                              { kind: 'EqualsZero' },
                              { kind: 'BranchIf'; id: '$B6' },
                              { kind: 'Const'; value: 8 },
                              { kind: 'LocalSet'; id: '$l61' },
                              { kind: 'LocalGet'; id: '$p0' },
                              { kind: 'LocalGet'; id: '$l61' },
                              { kind: 'Add' },
                              { kind: 'LocalSet'; id: '$l62' },
                              { kind: 'LocalGet'; id: '$l6' },
                              { kind: 'I32Load'; offset: 8 },
                              { kind: 'LocalSet'; id: '$l63' },
                              { kind: 'Const'; value: 36 },
                              { kind: 'LocalSet'; id: '$l64' },
                              { kind: 'LocalGet'; id: '$l63' },
                              { kind: 'LocalGet'; id: '$l64' },
                              { kind: 'Multiply' },
                              { kind: 'LocalSet'; id: '$l65' },
                              { kind: 'LocalGet'; id: '$l62' },
                              { kind: 'LocalGet'; id: '$l65' },
                              { kind: 'Add' },
                              { kind: 'LocalSet'; id: '$l66' },
                              { kind: 'LocalGet'; id: '$l6' },
                              { kind: 'I32Load'; offset: 4 },
                              { kind: 'LocalSet'; id: '$l67' },
                              { kind: 'Const'; value: 2 },
                              { kind: 'LocalSet'; id: '$l68' },
                              { kind: 'LocalGet'; id: '$l67' },
                              { kind: 'LocalGet'; id: '$l68' },
                              { kind: 'ShiftLeft' },
                              { kind: 'LocalSet'; id: '$l69' },
                              { kind: 'LocalGet'; id: '$l66' },
                              { kind: 'LocalGet'; id: '$l69' },
                              { kind: 'Add' },
                              { kind: 'LocalSet'; id: '$l70' },
                              { kind: 'LocalGet'; id: '$l70' },
                              { kind: 'I32Load'; offset: 0 },
                              { kind: 'LocalSet'; id: '$l71' },
                              { kind: 'LocalGet'; id: '$l71' },
                              { kind: 'EqualsZero' },
                              { kind: 'BranchIf'; id: '$B6' },
                              { kind: 'LocalGet'; id: '$l6' },
                              { kind: 'I32Load'; offset: 20 },
                              { kind: 'LocalSet'; id: '$l72' },
                              { kind: 'LocalGet'; id: '$l72' },
                              { kind: 'I32Load'; offset: 0 },
                              { kind: 'LocalSet'; id: '$l73' },
                              { kind: 'Const'; value: 1 },
                              { kind: 'LocalSet'; id: '$l74' },
                              { kind: 'LocalGet'; id: '$l73' },
                              { kind: 'LocalGet'; id: '$l74' },
                              { kind: 'Add' },
                              { kind: 'LocalSet'; id: '$l75' },
                              { kind: 'LocalGet'; id: '$l72' },
                              { kind: 'LocalGet'; id: '$l75' },
                              { kind: 'I32Store'; offset: 0 },
                            ];
                          },
                        ];
                      },
                      { kind: 'LocalGet'; id: '$l6' },
                      { kind: 'I32Load'; offset: 12 },
                      { kind: 'LocalSet'; id: '$l76' },
                      { kind: 'Const'; value: 1 },
                      { kind: 'LocalSet'; id: '$l77' },
                      { kind: 'LocalGet'; id: '$l76' },
                      { kind: 'LocalGet'; id: '$l77' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l78' },
                      { kind: 'LocalGet'; id: '$l6' },
                      { kind: 'LocalGet'; id: '$l78' },
                      { kind: 'I32Store'; offset: 12 },
                      { kind: 'Branch'; id: '$L3' },
                    ];
                  },
                  { kind: 'Unreachable' },
                ];
              },
              { kind: 'LocalGet'; id: '$l6' },
              { kind: 'I32Load'; offset: 16 },
              { kind: 'LocalSet'; id: '$l79' },
              { kind: 'Const'; value: 1 },
              { kind: 'LocalSet'; id: '$l80' },
              { kind: 'LocalGet'; id: '$l79' },
              { kind: 'LocalGet'; id: '$l80' },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$l81' },
              { kind: 'LocalGet'; id: '$l6' },
              { kind: 'LocalGet'; id: '$l81' },
              { kind: 'I32Store'; offset: 16 },
              { kind: 'Branch'; id: '$L1' },
            ];
          },
          { kind: 'Unreachable' },
        ];
      },
      { kind: 'Return'; count: 0 },
    ];
  }
> = RESULT

type $updateGrid<
  RESULT extends Func = {
    kind: 'func';
    params: ['$p0', '$p1'];
    result: number;
    locals: ['$l2', '$l3', '$l4', '$l5', '$l6', '$l7', '$l8', '$l9', '$l10', '$l11', '$l12', '$l13', '$l14', '$l15', '$l16', '$l17', '$l18', '$l19', '$l20', '$l21', '$l22', '$l23', '$l24', '$l25', '$l26', '$l27', '$l28', '$l29', '$l30', '$l31', '$l32', '$l33', '$l34', '$l35', '$l36', '$l37', '$l38', '$l39', '$l40', '$l41', '$l42', '$l43', '$l44', '$l45', '$l46', '$l47', '$l48', '$l49', '$l50', '$l51', '$l52', '$l53', '$l54', '$l55', '$l56', '$l57', '$l58', '$l59', '$l60', '$l61', '$l62', '$l63', '$l64', '$l65', '$l66', '$l67', '$l68', '$l69', '$l70', '$l71', '$l72', '$l73', '$l74', '$l75', '$l76', '$l77', '$l78', '$l79', '$l80', '$l81', '$l82', '$l83', '$l84', '$l85', '$l86', '$l87', '$l88', '$l89', '$l90', '$l91', '$l92', '$l93', '$l94', '$l95', '$l96', '$l97', '$l98', '$l99', '$l100'];
    instructions: [
      { kind: 'GlobalGet'; id: '$__stack_pointer' },
      { kind: 'LocalSet'; id: '$l2' },
      { kind: 'Const'; value: 352 },
      { kind: 'LocalSet'; id: '$l3' },
      { kind: 'LocalGet'; id: '$l2' },
      { kind: 'LocalGet'; id: '$l3' },
      { kind: 'Subtract' },
      { kind: 'LocalSet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'GlobalSet'; id: '$__stack_pointer' },
      { kind: 'LocalGet'; id: '$p1' },
      { kind: 'I32Load'; offset: 0 },
      { kind: 'LocalSet'; id: '$l5' },
      { kind: 'LocalGet'; id: '$p0' },
      { kind: 'LocalGet'; id: '$l5' },
      { kind: 'I32Store'; offset: 0 },
      { kind: 'LocalGet'; id: '$p1' },
      { kind: 'I32Load'; offset: 4 },
      { kind: 'LocalSet'; id: '$l6' },
      { kind: 'LocalGet'; id: '$p0' },
      { kind: 'LocalGet'; id: '$l6' },
      { kind: 'I32Store'; offset: 4 },
      { kind: 'Const'; value: 0 },
      { kind: 'LocalSet'; id: '$l7' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l7' },
      { kind: 'I32Store'; offset: 348 },
      { kind: 'Block';
        id: '$B0';
        instructions: [
          { kind: 'Loop';
            id: '$L1';
            instructions: [
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'I32Load'; offset: 348 },
              { kind: 'LocalSet'; id: '$l8' },
              { kind: 'LocalGet'; id: '$p1' },
              { kind: 'I32Load'; offset: 0 },
              { kind: 'LocalSet'; id: '$l9' },
              { kind: 'LocalGet'; id: '$l8' },
              { kind: 'LocalSet'; id: '$l10' },
              { kind: 'LocalGet'; id: '$l9' },
              { kind: 'LocalSet'; id: '$l11' },
              { kind: 'LocalGet'; id: '$l10' },
              { kind: 'LocalGet'; id: '$l11' },
              { kind: 'LessThan' },
              { kind: 'LocalSet'; id: '$l12' },
              { kind: 'Const'; value: 1 },
              { kind: 'LocalSet'; id: '$l13' },
              { kind: 'LocalGet'; id: '$l12' },
              { kind: 'LocalGet'; id: '$l13' },
              { kind: 'And' },
              { kind: 'LocalSet'; id: '$l14' },
              { kind: 'LocalGet'; id: '$l14' },
              { kind: 'EqualsZero' },
              { kind: 'BranchIf'; id: '$B0' },
              { kind: 'Const'; value: 0 },
              { kind: 'LocalSet'; id: '$l15' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'LocalGet'; id: '$l15' },
              { kind: 'I32Store'; offset: 344 },
              { kind: 'Block';
                id: '$B2';
                instructions: [
                  { kind: 'Loop';
                    id: '$L3';
                    instructions: [
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 344 },
                      { kind: 'LocalSet'; id: '$l16' },
                      { kind: 'LocalGet'; id: '$p1' },
                      { kind: 'I32Load'; offset: 4 },
                      { kind: 'LocalSet'; id: '$l17' },
                      { kind: 'LocalGet'; id: '$l16' },
                      { kind: 'LocalSet'; id: '$l18' },
                      { kind: 'LocalGet'; id: '$l17' },
                      { kind: 'LocalSet'; id: '$l19' },
                      { kind: 'LocalGet'; id: '$l18' },
                      { kind: 'LocalGet'; id: '$l19' },
                      { kind: 'LessThan' },
                      { kind: 'LocalSet'; id: '$l20' },
                      { kind: 'Const'; value: 1 },
                      { kind: 'LocalSet'; id: '$l21' },
                      { kind: 'LocalGet'; id: '$l20' },
                      { kind: 'LocalGet'; id: '$l21' },
                      { kind: 'And' },
                      { kind: 'LocalSet'; id: '$l22' },
                      { kind: 'LocalGet'; id: '$l22' },
                      { kind: 'EqualsZero' },
                      { kind: 'BranchIf'; id: '$B2' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 348 },
                      { kind: 'LocalSet'; id: '$l23' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 344 },
                      { kind: 'LocalSet'; id: '$l24' },
                      { kind: 'Const'; value: 332 },
                      { kind: 'LocalSet'; id: '$l25' },
                      { kind: 'Const'; value: 8 },
                      { kind: 'LocalSet'; id: '$l26' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'LocalGet'; id: '$l26' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l27' },
                      { kind: 'LocalGet'; id: '$l27' },
                      { kind: 'LocalGet'; id: '$p1' },
                      { kind: 'LocalGet'; id: '$l25' },
                      { kind: 'Call'; id: '$__memcpy' },
                      { kind: 'Drop' },
                      { kind: 'Const'; value: 8 },
                      { kind: 'LocalSet'; id: '$l28' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'LocalGet'; id: '$l28' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l29' },
                      { kind: 'Const'; value: 340 },
                      { kind: 'LocalSet'; id: '$l30' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'LocalGet'; id: '$l30' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l31' },
                      { kind: 'LocalGet'; id: '$l29' },
                      { kind: 'LocalGet'; id: '$l23' },
                      { kind: 'LocalGet'; id: '$l24' },
                      { kind: 'LocalGet'; id: '$l31' },
                      { kind: 'Call'; id: '$countAliveNeighbors' },
                      { kind: 'Const'; value: 8 },
                      { kind: 'LocalSet'; id: '$l32' },
                      { kind: 'LocalGet'; id: '$p1' },
                      { kind: 'LocalGet'; id: '$l32' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l33' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 348 },
                      { kind: 'LocalSet'; id: '$l34' },
                      { kind: 'Const'; value: 36 },
                      { kind: 'LocalSet'; id: '$l35' },
                      { kind: 'LocalGet'; id: '$l34' },
                      { kind: 'LocalGet'; id: '$l35' },
                      { kind: 'Multiply' },
                      { kind: 'LocalSet'; id: '$l36' },
                      { kind: 'LocalGet'; id: '$l33' },
                      { kind: 'LocalGet'; id: '$l36' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l37' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 344 },
                      { kind: 'LocalSet'; id: '$l38' },
                      { kind: 'Const'; value: 2 },
                      { kind: 'LocalSet'; id: '$l39' },
                      { kind: 'LocalGet'; id: '$l38' },
                      { kind: 'LocalGet'; id: '$l39' },
                      { kind: 'ShiftLeft' },
                      { kind: 'LocalSet'; id: '$l40' },
                      { kind: 'LocalGet'; id: '$l37' },
                      { kind: 'LocalGet'; id: '$l40' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l41' },
                      { kind: 'LocalGet'; id: '$l41' },
                      { kind: 'I32Load'; offset: 0 },
                      { kind: 'LocalSet'; id: '$l42' },
                      { kind: 'Block';
                        id: '$B4';
                        instructions: [
                          { kind: 'Block';
                            id: '$B5';
                            instructions: [
                              { kind: 'LocalGet'; id: '$l42' },
                              { kind: 'EqualsZero' },
                              { kind: 'BranchIf'; id: '$B5' },
                              { kind: 'LocalGet'; id: '$l4' },
                              { kind: 'I32Load'; offset: 340 },
                              { kind: 'LocalSet'; id: '$l43' },
                              { kind: 'Const'; value: 2 },
                              { kind: 'LocalSet'; id: '$l44' },
                              { kind: 'LocalGet'; id: '$l43' },
                              { kind: 'LocalSet'; id: '$l45' },
                              { kind: 'LocalGet'; id: '$l44' },
                              { kind: 'LocalSet'; id: '$l46' },
                              { kind: 'LocalGet'; id: '$l45' },
                              { kind: 'LocalGet'; id: '$l46' },
                              { kind: 'Equals' },
                              { kind: 'LocalSet'; id: '$l47' },
                              { kind: 'Const'; value: 1 },
                              { kind: 'LocalSet'; id: '$l48' },
                              { kind: 'Const'; value: 1 },
                              { kind: 'LocalSet'; id: '$l49' },
                              { kind: 'LocalGet'; id: '$l47' },
                              { kind: 'LocalGet'; id: '$l49' },
                              { kind: 'And' },
                              { kind: 'LocalSet'; id: '$l50' },
                              { kind: 'LocalGet'; id: '$l48' },
                              { kind: 'LocalSet'; id: '$l51' },
                              { kind: 'Block';
                                id: '$B6';
                                instructions: [
                                  { kind: 'LocalGet'; id: '$l50' },
                                  { kind: 'BranchIf'; id: '$B6' },
                                  { kind: 'LocalGet'; id: '$l4' },
                                  { kind: 'I32Load'; offset: 340 },
                                  { kind: 'LocalSet'; id: '$l52' },
                                  { kind: 'Const'; value: 3 },
                                  { kind: 'LocalSet'; id: '$l53' },
                                  { kind: 'LocalGet'; id: '$l52' },
                                  { kind: 'LocalSet'; id: '$l54' },
                                  { kind: 'LocalGet'; id: '$l53' },
                                  { kind: 'LocalSet'; id: '$l55' },
                                  { kind: 'LocalGet'; id: '$l54' },
                                  { kind: 'LocalGet'; id: '$l55' },
                                  { kind: 'Equals' },
                                  { kind: 'LocalSet'; id: '$l56' },
                                  { kind: 'LocalGet'; id: '$l56' },
                                  { kind: 'LocalSet'; id: '$l51' },
                                ];
                              },
                              { kind: 'LocalGet'; id: '$l51' },
                              { kind: 'LocalSet'; id: '$l57' },
                              { kind: 'Const'; value: 1 },
                              { kind: 'LocalSet'; id: '$l58' },
                              { kind: 'Const'; value: 0 },
                              { kind: 'LocalSet'; id: '$l59' },
                              { kind: 'Const'; value: 1 },
                              { kind: 'LocalSet'; id: '$l60' },
                              { kind: 'LocalGet'; id: '$l57' },
                              { kind: 'LocalGet'; id: '$l60' },
                              { kind: 'And' },
                              { kind: 'LocalSet'; id: '$l61' },
                              { kind: 'LocalGet'; id: '$l58' },
                              { kind: 'LocalGet'; id: '$l59' },
                              { kind: 'LocalGet'; id: '$l61' },
                              { kind: 'Select' },
                              { kind: 'LocalSet'; id: '$l62' },
                              { kind: 'Const'; value: 8 },
                              { kind: 'LocalSet'; id: '$l63' },
                              { kind: 'LocalGet'; id: '$p0' },
                              { kind: 'LocalGet'; id: '$l63' },
                              { kind: 'Add' },
                              { kind: 'LocalSet'; id: '$l64' },
                              { kind: 'LocalGet'; id: '$l4' },
                              { kind: 'I32Load'; offset: 348 },
                              { kind: 'LocalSet'; id: '$l65' },
                              { kind: 'Const'; value: 36 },
                              { kind: 'LocalSet'; id: '$l66' },
                              { kind: 'LocalGet'; id: '$l65' },
                              { kind: 'LocalGet'; id: '$l66' },
                              { kind: 'Multiply' },
                              { kind: 'LocalSet'; id: '$l67' },
                              { kind: 'LocalGet'; id: '$l64' },
                              { kind: 'LocalGet'; id: '$l67' },
                              { kind: 'Add' },
                              { kind: 'LocalSet'; id: '$l68' },
                              { kind: 'LocalGet'; id: '$l4' },
                              { kind: 'I32Load'; offset: 344 },
                              { kind: 'LocalSet'; id: '$l69' },
                              { kind: 'Const'; value: 2 },
                              { kind: 'LocalSet'; id: '$l70' },
                              { kind: 'LocalGet'; id: '$l69' },
                              { kind: 'LocalGet'; id: '$l70' },
                              { kind: 'ShiftLeft' },
                              { kind: 'LocalSet'; id: '$l71' },
                              { kind: 'LocalGet'; id: '$l68' },
                              { kind: 'LocalGet'; id: '$l71' },
                              { kind: 'Add' },
                              { kind: 'LocalSet'; id: '$l72' },
                              { kind: 'LocalGet'; id: '$l72' },
                              { kind: 'LocalGet'; id: '$l62' },
                              { kind: 'I32Store'; offset: 0 },
                              { kind: 'Branch'; id: '$B4' },
                            ];
                          },
                          { kind: 'LocalGet'; id: '$l4' },
                          { kind: 'I32Load'; offset: 340 },
                          { kind: 'LocalSet'; id: '$l73' },
                          { kind: 'Const'; value: 3 },
                          { kind: 'LocalSet'; id: '$l74' },
                          { kind: 'LocalGet'; id: '$l73' },
                          { kind: 'LocalSet'; id: '$l75' },
                          { kind: 'LocalGet'; id: '$l74' },
                          { kind: 'LocalSet'; id: '$l76' },
                          { kind: 'LocalGet'; id: '$l75' },
                          { kind: 'LocalGet'; id: '$l76' },
                          { kind: 'Equals' },
                          { kind: 'LocalSet'; id: '$l77' },
                          { kind: 'Const'; value: 1 },
                          { kind: 'LocalSet'; id: '$l78' },
                          { kind: 'Const'; value: 0 },
                          { kind: 'LocalSet'; id: '$l79' },
                          { kind: 'Const'; value: 1 },
                          { kind: 'LocalSet'; id: '$l80' },
                          { kind: 'LocalGet'; id: '$l77' },
                          { kind: 'LocalGet'; id: '$l80' },
                          { kind: 'And' },
                          { kind: 'LocalSet'; id: '$l81' },
                          { kind: 'LocalGet'; id: '$l78' },
                          { kind: 'LocalGet'; id: '$l79' },
                          { kind: 'LocalGet'; id: '$l81' },
                          { kind: 'Select' },
                          { kind: 'LocalSet'; id: '$l82' },
                          { kind: 'Const'; value: 8 },
                          { kind: 'LocalSet'; id: '$l83' },
                          { kind: 'LocalGet'; id: '$p0' },
                          { kind: 'LocalGet'; id: '$l83' },
                          { kind: 'Add' },
                          { kind: 'LocalSet'; id: '$l84' },
                          { kind: 'LocalGet'; id: '$l4' },
                          { kind: 'I32Load'; offset: 348 },
                          { kind: 'LocalSet'; id: '$l85' },
                          { kind: 'Const'; value: 36 },
                          { kind: 'LocalSet'; id: '$l86' },
                          { kind: 'LocalGet'; id: '$l85' },
                          { kind: 'LocalGet'; id: '$l86' },
                          { kind: 'Multiply' },
                          { kind: 'LocalSet'; id: '$l87' },
                          { kind: 'LocalGet'; id: '$l84' },
                          { kind: 'LocalGet'; id: '$l87' },
                          { kind: 'Add' },
                          { kind: 'LocalSet'; id: '$l88' },
                          { kind: 'LocalGet'; id: '$l4' },
                          { kind: 'I32Load'; offset: 344 },
                          { kind: 'LocalSet'; id: '$l89' },
                          { kind: 'Const'; value: 2 },
                          { kind: 'LocalSet'; id: '$l90' },
                          { kind: 'LocalGet'; id: '$l89' },
                          { kind: 'LocalGet'; id: '$l90' },
                          { kind: 'ShiftLeft' },
                          { kind: 'LocalSet'; id: '$l91' },
                          { kind: 'LocalGet'; id: '$l88' },
                          { kind: 'LocalGet'; id: '$l91' },
                          { kind: 'Add' },
                          { kind: 'LocalSet'; id: '$l92' },
                          { kind: 'LocalGet'; id: '$l92' },
                          { kind: 'LocalGet'; id: '$l82' },
                          { kind: 'I32Store'; offset: 0 },
                        ];
                      },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 344 },
                      { kind: 'LocalSet'; id: '$l93' },
                      { kind: 'Const'; value: 1 },
                      { kind: 'LocalSet'; id: '$l94' },
                      { kind: 'LocalGet'; id: '$l93' },
                      { kind: 'LocalGet'; id: '$l94' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l95' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'LocalGet'; id: '$l95' },
                      { kind: 'I32Store'; offset: 344 },
                      { kind: 'Branch'; id: '$L3' },
                    ];
                  },
                  { kind: 'Unreachable' },
                ];
              },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'I32Load'; offset: 348 },
              { kind: 'LocalSet'; id: '$l96' },
              { kind: 'Const'; value: 1 },
              { kind: 'LocalSet'; id: '$l97' },
              { kind: 'LocalGet'; id: '$l96' },
              { kind: 'LocalGet'; id: '$l97' },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$l98' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'LocalGet'; id: '$l98' },
              { kind: 'I32Store'; offset: 348 },
              { kind: 'Branch'; id: '$L1' },
            ];
          },
          { kind: 'Unreachable' },
        ];
      },
      { kind: 'Const'; value: 352 },
      { kind: 'LocalSet'; id: '$l99' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l99' },
      { kind: 'Add' },
      { kind: 'LocalSet'; id: '$l100' },
      { kind: 'LocalGet'; id: '$l100' },
      { kind: 'GlobalSet'; id: '$__stack_pointer' },
      { kind: 'Return'; count: 0 },
    ];
  }
> = RESULT

type $entry<
  RESULT extends Func = {
    kind: 'func';
    params: ['$p0', '$p1'];
    result: number;
    locals: ['$l2', '$l3', '$l4', '$l5', '$l6', '$l7', '$l8', '$l9', '$l10', '$l11', '$l12', '$l13', '$l14', '$l15', '$l16', '$l17', '$l18', '$l19', '$l20', '$l21', '$l22', '$l23', '$l24', '$l25', '$l26', '$l27', '$l28', '$l29', '$l30', '$l31', '$l32', '$l33', '$l34', '$l35', '$l36', '$l37', '$l38', '$l39', '$l40', '$l41', '$l42', '$l43', '$l44', '$l45', '$l46', '$l47', '$l48', '$l49', '$l50', '$l51', '$l52', '$l53', '$l54', '$l55', '$l56', '$l57', '$l58', '$l59', '$l60', '$l61', '$l62', '$l63', '$l64', '$l65', '$l66', '$l67', '$l68', '$l69', '$l70', '$l71', '$l72', '$l73', '$l74', '$l75', '$l76', '$l77', '$l78', '$l79', '$l80', '$l81', '$l82', '$l83', '$l84', '$l85', '$l86'];
    instructions: [
      { kind: 'GlobalGet'; id: '$__stack_pointer' },
      { kind: 'LocalSet'; id: '$l2' },
      { kind: 'Const'; value: 1024 },
      { kind: 'LocalSet'; id: '$l3' },
      { kind: 'LocalGet'; id: '$l2' },
      { kind: 'LocalGet'; id: '$l3' },
      { kind: 'Subtract' },
      { kind: 'LocalSet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'GlobalSet'; id: '$__stack_pointer' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$p1' },
      { kind: 'I32Store'; offset: 1020 },
      { kind: 'Const'; value: 9 },
      { kind: 'LocalSet'; id: '$l5' },
      { kind: 'LocalGet'; id: '$p0' },
      { kind: 'LocalGet'; id: '$l5' },
      { kind: 'I32Store'; offset: 0 },
      { kind: 'Const'; value: 9 },
      { kind: 'LocalSet'; id: '$l6' },
      { kind: 'LocalGet'; id: '$p0' },
      { kind: 'LocalGet'; id: '$l6' },
      { kind: 'I32Store'; offset: 4 },
      { kind: 'Const'; value: 688 },
      { kind: 'LocalSet'; id: '$l7' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l7' },
      { kind: 'Add' },
      { kind: 'LocalSet'; id: '$l8' },
      { kind: 'LocalGet'; id: '$l8' },
      { kind: 'LocalSet'; id: '$l9' },
      { kind: 'Const'; value: 324 },
      { kind: 'LocalSet'; id: '$l10' },
      { kind: 'Const'; value: 0 },
      { kind: 'LocalSet'; id: '$l11' },
      { kind: 'LocalGet'; id: '$l9' },
      { kind: 'LocalGet'; id: '$l11' },
      { kind: 'LocalGet'; id: '$l10' },
      { kind: 'Call'; id: '$memset' },
      { kind: 'Drop' },
      { kind: 'Const'; value: 1 },
      { kind: 'LocalSet'; id: '$l12' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l12' },
      { kind: 'I32Store'; offset: 812 },
      { kind: 'Const'; value: 1 },
      { kind: 'LocalSet'; id: '$l13' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l13' },
      { kind: 'I32Store'; offset: 844 },
      { kind: 'Const'; value: 1 },
      { kind: 'LocalSet'; id: '$l14' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l14' },
      { kind: 'I32Store'; offset: 848 },
      { kind: 'Const'; value: 1 },
      { kind: 'LocalSet'; id: '$l15' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l15' },
      { kind: 'I32Store'; offset: 852 },
      { kind: 'Const'; value: 1 },
      { kind: 'LocalSet'; id: '$l16' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l16' },
      { kind: 'I32Store'; offset: 884 },
      { kind: 'Const'; value: 0 },
      { kind: 'LocalSet'; id: '$l17' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l17' },
      { kind: 'I32Store'; offset: 684 },
      { kind: 'Block';
        id: '$B0';
        instructions: [
          { kind: 'Loop';
            id: '$L1';
            instructions: [
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'I32Load'; offset: 684 },
              { kind: 'LocalSet'; id: '$l18' },
              { kind: 'LocalGet'; id: '$p0' },
              { kind: 'I32Load'; offset: 0 },
              { kind: 'LocalSet'; id: '$l19' },
              { kind: 'LocalGet'; id: '$l18' },
              { kind: 'LocalSet'; id: '$l20' },
              { kind: 'LocalGet'; id: '$l19' },
              { kind: 'LocalSet'; id: '$l21' },
              { kind: 'LocalGet'; id: '$l20' },
              { kind: 'LocalGet'; id: '$l21' },
              { kind: 'LessThan' },
              { kind: 'LocalSet'; id: '$l22' },
              { kind: 'Const'; value: 1 },
              { kind: 'LocalSet'; id: '$l23' },
              { kind: 'LocalGet'; id: '$l22' },
              { kind: 'LocalGet'; id: '$l23' },
              { kind: 'And' },
              { kind: 'LocalSet'; id: '$l24' },
              { kind: 'LocalGet'; id: '$l24' },
              { kind: 'EqualsZero' },
              { kind: 'BranchIf'; id: '$B0' },
              { kind: 'Const'; value: 0 },
              { kind: 'LocalSet'; id: '$l25' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'LocalGet'; id: '$l25' },
              { kind: 'I32Store'; offset: 680 },
              { kind: 'Block';
                id: '$B2';
                instructions: [
                  { kind: 'Loop';
                    id: '$L3';
                    instructions: [
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 680 },
                      { kind: 'LocalSet'; id: '$l26' },
                      { kind: 'LocalGet'; id: '$p0' },
                      { kind: 'I32Load'; offset: 4 },
                      { kind: 'LocalSet'; id: '$l27' },
                      { kind: 'LocalGet'; id: '$l26' },
                      { kind: 'LocalSet'; id: '$l28' },
                      { kind: 'LocalGet'; id: '$l27' },
                      { kind: 'LocalSet'; id: '$l29' },
                      { kind: 'LocalGet'; id: '$l28' },
                      { kind: 'LocalGet'; id: '$l29' },
                      { kind: 'LessThan' },
                      { kind: 'LocalSet'; id: '$l30' },
                      { kind: 'Const'; value: 1 },
                      { kind: 'LocalSet'; id: '$l31' },
                      { kind: 'LocalGet'; id: '$l30' },
                      { kind: 'LocalGet'; id: '$l31' },
                      { kind: 'And' },
                      { kind: 'LocalSet'; id: '$l32' },
                      { kind: 'LocalGet'; id: '$l32' },
                      { kind: 'EqualsZero' },
                      { kind: 'BranchIf'; id: '$B2' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 684 },
                      { kind: 'LocalSet'; id: '$l33' },
                      { kind: 'Const'; value: 688 },
                      { kind: 'LocalSet'; id: '$l34' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'LocalGet'; id: '$l34' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l35' },
                      { kind: 'LocalGet'; id: '$l35' },
                      { kind: 'LocalSet'; id: '$l36' },
                      { kind: 'Const'; value: 36 },
                      { kind: 'LocalSet'; id: '$l37' },
                      { kind: 'LocalGet'; id: '$l33' },
                      { kind: 'LocalGet'; id: '$l37' },
                      { kind: 'Multiply' },
                      { kind: 'LocalSet'; id: '$l38' },
                      { kind: 'LocalGet'; id: '$l36' },
                      { kind: 'LocalGet'; id: '$l38' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l39' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 680 },
                      { kind: 'LocalSet'; id: '$l40' },
                      { kind: 'Const'; value: 2 },
                      { kind: 'LocalSet'; id: '$l41' },
                      { kind: 'LocalGet'; id: '$l40' },
                      { kind: 'LocalGet'; id: '$l41' },
                      { kind: 'ShiftLeft' },
                      { kind: 'LocalSet'; id: '$l42' },
                      { kind: 'LocalGet'; id: '$l39' },
                      { kind: 'LocalGet'; id: '$l42' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l43' },
                      { kind: 'LocalGet'; id: '$l43' },
                      { kind: 'I32Load'; offset: 0 },
                      { kind: 'LocalSet'; id: '$l44' },
                      { kind: 'Const'; value: 8 },
                      { kind: 'LocalSet'; id: '$l45' },
                      { kind: 'LocalGet'; id: '$p0' },
                      { kind: 'LocalGet'; id: '$l45' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l46' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 684 },
                      { kind: 'LocalSet'; id: '$l47' },
                      { kind: 'Const'; value: 36 },
                      { kind: 'LocalSet'; id: '$l48' },
                      { kind: 'LocalGet'; id: '$l47' },
                      { kind: 'LocalGet'; id: '$l48' },
                      { kind: 'Multiply' },
                      { kind: 'LocalSet'; id: '$l49' },
                      { kind: 'LocalGet'; id: '$l46' },
                      { kind: 'LocalGet'; id: '$l49' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l50' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 680 },
                      { kind: 'LocalSet'; id: '$l51' },
                      { kind: 'Const'; value: 2 },
                      { kind: 'LocalSet'; id: '$l52' },
                      { kind: 'LocalGet'; id: '$l51' },
                      { kind: 'LocalGet'; id: '$l52' },
                      { kind: 'ShiftLeft' },
                      { kind: 'LocalSet'; id: '$l53' },
                      { kind: 'LocalGet'; id: '$l50' },
                      { kind: 'LocalGet'; id: '$l53' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l54' },
                      { kind: 'LocalGet'; id: '$l54' },
                      { kind: 'LocalGet'; id: '$l44' },
                      { kind: 'I32Store'; offset: 0 },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'I32Load'; offset: 680 },
                      { kind: 'LocalSet'; id: '$l55' },
                      { kind: 'Const'; value: 1 },
                      { kind: 'LocalSet'; id: '$l56' },
                      { kind: 'LocalGet'; id: '$l55' },
                      { kind: 'LocalGet'; id: '$l56' },
                      { kind: 'Add' },
                      { kind: 'LocalSet'; id: '$l57' },
                      { kind: 'LocalGet'; id: '$l4' },
                      { kind: 'LocalGet'; id: '$l57' },
                      { kind: 'I32Store'; offset: 680 },
                      { kind: 'Branch'; id: '$L3' },
                    ];
                  },
                  { kind: 'Unreachable' },
                ];
              },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'I32Load'; offset: 684 },
              { kind: 'LocalSet'; id: '$l58' },
              { kind: 'Const'; value: 1 },
              { kind: 'LocalSet'; id: '$l59' },
              { kind: 'LocalGet'; id: '$l58' },
              { kind: 'LocalGet'; id: '$l59' },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$l60' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'LocalGet'; id: '$l60' },
              { kind: 'I32Store'; offset: 684 },
              { kind: 'Branch'; id: '$L1' },
            ];
          },
          { kind: 'Unreachable' },
        ];
      },
      { kind: 'Const'; value: 0 },
      { kind: 'LocalSet'; id: '$l61' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l61' },
      { kind: 'I32Store'; offset: 676 },
      { kind: 'Block';
        id: '$B4';
        instructions: [
          { kind: 'Loop';
            id: '$L5';
            instructions: [
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'I32Load'; offset: 676 },
              { kind: 'LocalSet'; id: '$l62' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'I32Load'; offset: 1020 },
              { kind: 'LocalSet'; id: '$l63' },
              { kind: 'LocalGet'; id: '$l62' },
              { kind: 'LocalSet'; id: '$l64' },
              { kind: 'LocalGet'; id: '$l63' },
              { kind: 'LocalSet'; id: '$l65' },
              { kind: 'LocalGet'; id: '$l64' },
              { kind: 'LocalGet'; id: '$l65' },
              { kind: 'LessThan' },
              { kind: 'LocalSet'; id: '$l66' },
              { kind: 'Const'; value: 1 },
              { kind: 'LocalSet'; id: '$l67' },
              { kind: 'LocalGet'; id: '$l66' },
              { kind: 'LocalGet'; id: '$l67' },
              { kind: 'And' },
              { kind: 'LocalSet'; id: '$l68' },
              { kind: 'LocalGet'; id: '$l68' },
              { kind: 'EqualsZero' },
              { kind: 'BranchIf'; id: '$B4' },
              { kind: 'Const'; value: 344 },
              { kind: 'LocalSet'; id: '$l69' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'LocalGet'; id: '$l69' },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$l70' },
              { kind: 'LocalGet'; id: '$l70' },
              { kind: 'Drop' },
              { kind: 'Const'; value: 332 },
              { kind: 'LocalSet'; id: '$l71' },
              { kind: 'Const'; value: 12 },
              { kind: 'LocalSet'; id: '$l72' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'LocalGet'; id: '$l72' },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$l73' },
              { kind: 'LocalGet'; id: '$l73' },
              { kind: 'LocalGet'; id: '$p0' },
              { kind: 'LocalGet'; id: '$l71' },
              { kind: 'Call'; id: '$__memcpy' },
              { kind: 'Drop' },
              { kind: 'Const'; value: 344 },
              { kind: 'LocalSet'; id: '$l74' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'LocalGet'; id: '$l74' },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$l75' },
              { kind: 'Const'; value: 12 },
              { kind: 'LocalSet'; id: '$l76' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'LocalGet'; id: '$l76' },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$l77' },
              { kind: 'LocalGet'; id: '$l75' },
              { kind: 'LocalGet'; id: '$l77' },
              { kind: 'Call'; id: '$updateGrid' },
              { kind: 'Const'; value: 344 },
              { kind: 'LocalSet'; id: '$l78' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'LocalGet'; id: '$l78' },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$l79' },
              { kind: 'LocalGet'; id: '$l79' },
              { kind: 'LocalSet'; id: '$l80' },
              { kind: 'Const'; value: 332 },
              { kind: 'LocalSet'; id: '$l81' },
              { kind: 'LocalGet'; id: '$p0' },
              { kind: 'LocalGet'; id: '$l80' },
              { kind: 'LocalGet'; id: '$l81' },
              { kind: 'Call'; id: '$__memcpy' },
              { kind: 'Drop' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'I32Load'; offset: 676 },
              { kind: 'LocalSet'; id: '$l82' },
              { kind: 'Const'; value: 1 },
              { kind: 'LocalSet'; id: '$l83' },
              { kind: 'LocalGet'; id: '$l82' },
              { kind: 'LocalGet'; id: '$l83' },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$l84' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'LocalGet'; id: '$l84' },
              { kind: 'I32Store'; offset: 676 },
              { kind: 'Branch'; id: '$L5' },
            ];
          },
          { kind: 'Unreachable' },
        ];
      },
      { kind: 'Const'; value: 1024 },
      { kind: 'LocalSet'; id: '$l85' },
      { kind: 'LocalGet'; id: '$l4' },
      { kind: 'LocalGet'; id: '$l85' },
      { kind: 'Add' },
      { kind: 'LocalSet'; id: '$l86' },
      { kind: 'LocalGet'; id: '$l86' },
      { kind: 'GlobalSet'; id: '$__stack_pointer' },
      { kind: 'Return'; count: 0 },
    ];
  }
> = RESULT

type $_initialize<
  RESULT extends Func = {
    kind: 'func';
    params: [];
    result: number;
    locals: [];
    instructions: [
      { kind: 'Block';
        id: '$B0';
        instructions: [
          { kind: 'Const'; value: 1 },
          { kind: 'EqualsZero' },
          { kind: 'BranchIf'; id: '$B0' },
          { kind: 'Call'; id: '$__wasm_call_ctors' },
        ];
      },
    ];
  }
> = RESULT

type $__memcpy<
  RESULT extends Func = {
    kind: 'func';
    params: ['$p0', '$p1', '$p2'];
    result: number;
    locals: ['$l3', '$l4', '$l5'];
    instructions: [
      { kind: 'LocalGet'; id: '$p0' },
      { kind: 'LocalGet'; id: '$p2' },
      { kind: 'Add' },
      { kind: 'LocalSet'; id: '$l3' },
      { kind: 'Block';
        id: '$B0';
        instructions: [
          { kind: 'Block';
            id: '$B1';
            instructions: [
              { kind: 'Block';
                id: '$B2';
                instructions: [
                  { kind: 'Block';
                    id: '$B3';
                    instructions: [
                      { kind: 'LocalGet'; id: '$p1' },
                      { kind: 'LocalGet'; id: '$p0' },
                      { kind: 'Xor' },
                      { kind: 'Const'; value: 3 },
                      { kind: 'And' },
                      { kind: 'BranchIf'; id: '$B3' },
                      { kind: 'LocalGet'; id: '$p0' },
                      { kind: 'Const'; value: 3 },
                      { kind: 'And' },
                      { kind: 'EqualsZero' },
                      { kind: 'BranchIf'; id: '$B2' },
                      { kind: 'LocalGet'; id: '$p2' },
                      { kind: 'Const'; value: 1 },
                      { kind: 'LessThan' },
                      { kind: 'BranchIf'; id: '$B2' },
                      { kind: 'LocalGet'; id: '$p0' },
                      { kind: 'LocalSet'; id: '$p2' },
                      { kind: 'Loop';
                        id: '$L4';
                        instructions: [
                          { kind: 'LocalGet'; id: '$p2' },
                          { kind: 'LocalGet'; id: '$p1' },
                          { kind: 'I32Load8u'; offset: 0 },
                          { kind: 'I32Store8'; offset: 0 },
                          { kind: 'LocalGet'; id: '$p1' },
                          { kind: 'Const'; value: 1 },
                          { kind: 'Add' },
                          { kind: 'LocalSet'; id: '$p1' },
                          { kind: 'LocalGet'; id: '$p2' },
                          { kind: 'Const'; value: 1 },
                          { kind: 'Add' },
                          { kind: 'LocalTee'; id: '$p2' },
                          { kind: 'Const'; value: 3 },
                          { kind: 'And' },
                          { kind: 'EqualsZero' },
                          { kind: 'BranchIf'; id: '$B1' },
                          { kind: 'LocalGet'; id: '$p2' },
                          { kind: 'LocalGet'; id: '$l3' },
                          { kind: 'LessThan' },
                          { kind: 'BranchIf'; id: '$L4' },
                          { kind: 'Branch'; id: '$B1' },
                        ];
                      },
                      { kind: 'Unreachable' },
                    ];
                  },
                  { kind: 'Block';
                    id: '$B5';
                    instructions: [
                      { kind: 'LocalGet'; id: '$l3' },
                      { kind: 'Const'; value: 4 },
                      { kind: 'LessThan' },
                      { kind: 'BranchIf'; id: '$B5' },
                      { kind: 'LocalGet'; id: '$l3' },
                      { kind: 'Const'; value: -4 },
                      { kind: 'Add' },
                      { kind: 'LocalTee'; id: '$l4' },
                      { kind: 'LocalGet'; id: '$p0' },
                      { kind: 'LessThan' },
                      { kind: 'BranchIf'; id: '$B5' },
                      { kind: 'LocalGet'; id: '$p0' },
                      { kind: 'LocalSet'; id: '$p2' },
                      { kind: 'Loop';
                        id: '$L6';
                        instructions: [
                          { kind: 'LocalGet'; id: '$p2' },
                          { kind: 'LocalGet'; id: '$p1' },
                          { kind: 'I32Load8u'; offset: 0 },
                          { kind: 'I32Store8'; offset: 0 },
                          { kind: 'LocalGet'; id: '$p2' },
                          { kind: 'LocalGet'; id: '$p1' },
                          { kind: 'I32Load8u'; offset: 1 },
                          { kind: 'I32Store8'; offset: 1 },
                          { kind: 'LocalGet'; id: '$p2' },
                          { kind: 'LocalGet'; id: '$p1' },
                          { kind: 'I32Load8u'; offset: 2 },
                          { kind: 'I32Store8'; offset: 2 },
                          { kind: 'LocalGet'; id: '$p2' },
                          { kind: 'LocalGet'; id: '$p1' },
                          { kind: 'I32Load8u'; offset: 3 },
                          { kind: 'I32Store8'; offset: 3 },
                          { kind: 'LocalGet'; id: '$p1' },
                          { kind: 'Const'; value: 4 },
                          { kind: 'Add' },
                          { kind: 'LocalSet'; id: '$p1' },
                          { kind: 'LocalGet'; id: '$p2' },
                          { kind: 'Const'; value: 4 },
                          { kind: 'Add' },
                          { kind: 'LocalTee'; id: '$p2' },
                          { kind: 'LocalGet'; id: '$l4' },
                          { kind: 'LessThanOrEqual' },
                          { kind: 'BranchIf'; id: '$L6' },
                          { kind: 'Branch'; id: '$B0' },
                        ];
                      },
                      { kind: 'Unreachable' },
                    ];
                  },
                  { kind: 'LocalGet'; id: '$p0' },
                  { kind: 'LocalSet'; id: '$p2' },
                  { kind: 'Branch'; id: '$B0' },
                ];
              },
              { kind: 'LocalGet'; id: '$p0' },
              { kind: 'LocalSet'; id: '$p2' },
            ];
          },
          { kind: 'Block';
            id: '$B7';
            instructions: [
              { kind: 'LocalGet'; id: '$l3' },
              { kind: 'Const'; value: -4 },
              { kind: 'And' },
              { kind: 'LocalTee'; id: '$l4' },
              { kind: 'Const'; value: 64 },
              { kind: 'LessThan' },
              { kind: 'BranchIf'; id: '$B7' },
              { kind: 'LocalGet'; id: '$p2' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'Const'; value: -64 },
              { kind: 'Add' },
              { kind: 'LocalTee'; id: '$l5' },
              { kind: 'GreaterThan' },
              { kind: 'BranchIf'; id: '$B7' },
              { kind: 'Loop';
                id: '$L8';
                instructions: [
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 0 },
                  { kind: 'I32Store'; offset: 0 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 4 },
                  { kind: 'I32Store'; offset: 4 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 8 },
                  { kind: 'I32Store'; offset: 8 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 12 },
                  { kind: 'I32Store'; offset: 12 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 16 },
                  { kind: 'I32Store'; offset: 16 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 20 },
                  { kind: 'I32Store'; offset: 20 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 24 },
                  { kind: 'I32Store'; offset: 24 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 28 },
                  { kind: 'I32Store'; offset: 28 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 32 },
                  { kind: 'I32Store'; offset: 32 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 36 },
                  { kind: 'I32Store'; offset: 36 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 40 },
                  { kind: 'I32Store'; offset: 40 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 44 },
                  { kind: 'I32Store'; offset: 44 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 48 },
                  { kind: 'I32Store'; offset: 48 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 52 },
                  { kind: 'I32Store'; offset: 52 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 56 },
                  { kind: 'I32Store'; offset: 56 },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'I32Load'; offset: 60 },
                  { kind: 'I32Store'; offset: 60 },
                  { kind: 'LocalGet'; id: '$p1' },
                  { kind: 'Const'; value: 64 },
                  { kind: 'Add' },
                  { kind: 'LocalSet'; id: '$p1' },
                  { kind: 'LocalGet'; id: '$p2' },
                  { kind: 'Const'; value: 64 },
                  { kind: 'Add' },
                  { kind: 'LocalTee'; id: '$p2' },
                  { kind: 'LocalGet'; id: '$l5' },
                  { kind: 'LessThanOrEqual' },
                  { kind: 'BranchIf'; id: '$L8' },
                ];
              },
            ];
          },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'LocalGet'; id: '$l4' },
          { kind: 'GreaterThanOrEqual' },
          { kind: 'BranchIf'; id: '$B0' },
          { kind: 'Loop';
            id: '$L9';
            instructions: [
              { kind: 'LocalGet'; id: '$p2' },
              { kind: 'LocalGet'; id: '$p1' },
              { kind: 'I32Load'; offset: 0 },
              { kind: 'I32Store'; offset: 0 },
              { kind: 'LocalGet'; id: '$p1' },
              { kind: 'Const'; value: 4 },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$p1' },
              { kind: 'LocalGet'; id: '$p2' },
              { kind: 'Const'; value: 4 },
              { kind: 'Add' },
              { kind: 'LocalTee'; id: '$p2' },
              { kind: 'LocalGet'; id: '$l4' },
              { kind: 'LessThan' },
              { kind: 'BranchIf'; id: '$L9' },
            ];
          },
        ];
      },
      { kind: 'Block';
        id: '$B10';
        instructions: [
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'GreaterThanOrEqual' },
          { kind: 'BranchIf'; id: '$B10' },
          { kind: 'Loop';
            id: '$L11';
            instructions: [
              { kind: 'LocalGet'; id: '$p2' },
              { kind: 'LocalGet'; id: '$p1' },
              { kind: 'I32Load8u'; offset: 0 },
              { kind: 'I32Store8'; offset: 0 },
              { kind: 'LocalGet'; id: '$p1' },
              { kind: 'Const'; value: 1 },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$p1' },
              { kind: 'LocalGet'; id: '$p2' },
              { kind: 'Const'; value: 1 },
              { kind: 'Add' },
              { kind: 'LocalTee'; id: '$p2' },
              { kind: 'LocalGet'; id: '$l3' },
              { kind: 'NotEqual' },
              { kind: 'BranchIf'; id: '$L11' },
            ];
          },
        ];
      },
      { kind: 'LocalGet'; id: '$p0' },
    ];
  }
> = RESULT

type $memset<
  RESULT extends Func = {
    kind: 'func';
    params: ['$p0', '$p1', '$p2'];
    result: number;
    locals: ['$l3', '$l4', '$l5', '$l6'];
    instructions: [
      { kind: 'Block';
        id: '$B0';
        instructions: [
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'EqualsZero' },
          { kind: 'BranchIf'; id: '$B0' },
          { kind: 'LocalGet'; id: '$p0' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store8'; offset: 0 },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'LocalGet'; id: '$p0' },
          { kind: 'Add' },
          { kind: 'LocalTee'; id: '$l3' },
          { kind: 'Const'; value: -1 },
          { kind: 'Add' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store8'; offset: 0 },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'Const'; value: 3 },
          { kind: 'LessThan' },
          { kind: 'BranchIf'; id: '$B0' },
          { kind: 'LocalGet'; id: '$p0' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store8'; offset: 2 },
          { kind: 'LocalGet'; id: '$p0' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store8'; offset: 1 },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'Const'; value: -3 },
          { kind: 'Add' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store8'; offset: 0 },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'Const'; value: -2 },
          { kind: 'Add' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store8'; offset: 0 },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'Const'; value: 7 },
          { kind: 'LessThan' },
          { kind: 'BranchIf'; id: '$B0' },
          { kind: 'LocalGet'; id: '$p0' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store8'; offset: 3 },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'Const'; value: -4 },
          { kind: 'Add' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store8'; offset: 0 },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'Const'; value: 9 },
          { kind: 'LessThan' },
          { kind: 'BranchIf'; id: '$B0' },
          { kind: 'LocalGet'; id: '$p0' },
          { kind: 'Const'; value: 0 },
          { kind: 'LocalGet'; id: '$p0' },
          { kind: 'Subtract' },
          { kind: 'Const'; value: 3 },
          { kind: 'And' },
          { kind: 'LocalTee'; id: '$l4' },
          { kind: 'Add' },
          { kind: 'LocalTee'; id: '$l3' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'Const'; value: 255 },
          { kind: 'And' },
          { kind: 'Const'; value: 16843009 },
          { kind: 'Multiply' },
          { kind: 'LocalTee'; id: '$p1' },
          { kind: 'I32Store'; offset: 0 },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'LocalGet'; id: '$l4' },
          { kind: 'Subtract' },
          { kind: 'Const'; value: -4 },
          { kind: 'And' },
          { kind: 'LocalTee'; id: '$l4' },
          { kind: 'Add' },
          { kind: 'LocalTee'; id: '$p2' },
          { kind: 'Const'; value: -4 },
          { kind: 'Add' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 0 },
          { kind: 'LocalGet'; id: '$l4' },
          { kind: 'Const'; value: 9 },
          { kind: 'LessThan' },
          { kind: 'BranchIf'; id: '$B0' },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 8 },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 4 },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'Const'; value: -8 },
          { kind: 'Add' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 0 },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'Const'; value: -12 },
          { kind: 'Add' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 0 },
          { kind: 'LocalGet'; id: '$l4' },
          { kind: 'Const'; value: 25 },
          { kind: 'LessThan' },
          { kind: 'BranchIf'; id: '$B0' },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 24 },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 20 },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 16 },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 12 },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'Const'; value: -16 },
          { kind: 'Add' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 0 },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'Const'; value: -20 },
          { kind: 'Add' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 0 },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'Const'; value: -24 },
          { kind: 'Add' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 0 },
          { kind: 'LocalGet'; id: '$p2' },
          { kind: 'Const'; value: -28 },
          { kind: 'Add' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'I32Store'; offset: 0 },
          { kind: 'LocalGet'; id: '$l4' },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'Const'; value: 4 },
          { kind: 'And' },
          { kind: 'Const'; value: 24 },
          { kind: 'Or' },
          { kind: 'LocalTee'; id: '$l5' },
          { kind: 'Subtract' },
          { kind: 'LocalTee'; id: '$p2' },
          { kind: 'Const'; value: 32 },
          { kind: 'LessThan' },
          { kind: 'BranchIf'; id: '$B0' },
          { kind: 'LocalGet'; id: '$p1' },
          { kind: 'Extend8Unsigned' },
          { kind: 'Const'; value: 4294967297 },
          { kind: 'Multiply' },
          { kind: 'LocalSet'; id: '$l6' },
          { kind: 'LocalGet'; id: '$l3' },
          { kind: 'LocalGet'; id: '$l5' },
          { kind: 'Add' },
          { kind: 'LocalSet'; id: '$p1' },
          { kind: 'Loop';
            id: '$L1';
            instructions: [
              { kind: 'LocalGet'; id: '$p1' },
              { kind: 'LocalGet'; id: '$l6' },
              { kind: 'I64Store'; offset: 24 },
              { kind: 'LocalGet'; id: '$p1' },
              { kind: 'LocalGet'; id: '$l6' },
              { kind: 'I64Store'; offset: 16 },
              { kind: 'LocalGet'; id: '$p1' },
              { kind: 'LocalGet'; id: '$l6' },
              { kind: 'I64Store'; offset: 8 },
              { kind: 'LocalGet'; id: '$p1' },
              { kind: 'LocalGet'; id: '$l6' },
              { kind: 'I64Store'; offset: 0 },
              { kind: 'LocalGet'; id: '$p1' },
              { kind: 'Const'; value: 32 },
              { kind: 'Add' },
              { kind: 'LocalSet'; id: '$p1' },
              { kind: 'LocalGet'; id: '$p2' },
              { kind: 'Const'; value: -32 },
              { kind: 'Add' },
              { kind: 'LocalTee'; id: '$p2' },
              { kind: 'Const'; value: 31 },
              { kind: 'GreaterThan' },
              { kind: 'BranchIf'; id: '$L1' },
            ];
          },
        ];
      },
      { kind: 'LocalGet'; id: '$p0' },
    ];
  }
> = RESULT

type $stackSave<
  RESULT extends Func = {
    kind: 'func';
    params: [];
    result: number;
    locals: [];
    instructions: [
      { kind: 'GlobalGet'; id: '$__stack_pointer' },
    ];
  }
> = RESULT

type $stackRestore<
  RESULT extends Func = {
    kind: 'func';
    params: ['$p0'];
    result: number;
    locals: [];
    instructions: [
      { kind: 'LocalGet'; id: '$p0' },
      { kind: 'GlobalSet'; id: '$__stack_pointer' },
    ];
  }
> = RESULT

type $stackAlloc<
  RESULT extends Func = {
    kind: 'func';
    params: ['$p0'];
    result: number;
    locals: ['$l1', '$l2'];
    instructions: [
      { kind: 'GlobalGet'; id: '$__stack_pointer' },
      { kind: 'LocalGet'; id: '$p0' },
      { kind: 'Subtract' },
      { kind: 'Const'; value: -16 },
      { kind: 'And' },
      { kind: 'LocalTee'; id: '$l1' },
      { kind: 'GlobalSet'; id: '$__stack_pointer' },
      { kind: 'LocalGet'; id: '$l1' },
    ];
  }
> = RESULT

type $emscripten_stack_init<
  RESULT extends Func = {
    kind: 'func';
    params: [];
    result: number;
    locals: [];
    instructions: [
      { kind: 'Const'; value: 5243920 },
      { kind: 'GlobalSet'; id: '$__stack_base' },
      { kind: 'Const'; value: 1028 },
      { kind: 'Const'; value: 15 },
      { kind: 'Add' },
      { kind: 'Const'; value: -16 },
      { kind: 'And' },
      { kind: 'GlobalSet'; id: '$__stack_end' },
    ];
  }
> = RESULT

type $emscripten_stack_get_free<
  RESULT extends Func = {
    kind: 'func';
    params: [];
    result: number;
    locals: [];
    instructions: [
      { kind: 'GlobalGet'; id: '$__stack_pointer' },
      { kind: 'GlobalGet'; id: '$__stack_end' },
      { kind: 'Subtract' },
    ];
  }
> = RESULT

type $emscripten_stack_get_base<
  RESULT extends Func = {
    kind: 'func';
    params: [];
    result: number;
    locals: [];
    instructions: [
      { kind: 'GlobalGet'; id: '$__stack_base' },
    ];
  }
> = RESULT

type $emscripten_stack_get_end<
  RESULT extends Func = {
    kind: 'func';
    params: [];
    result: number;
    locals: [];
    instructions: [
      { kind: 'GlobalGet'; id: '$__stack_end' },
    ];
  }
> = RESULT

type $__errno_location<
  RESULT extends Func = {
    kind: 'func';
    params: [];
    result: number;
    locals: [];
    instructions: [
      { kind: 'Const'; value: 1024 },
    ];
  }
> = RESULT

export type entry<
  arguments extends number[] = [],
  debugMode extends boolean = false
> = runProgram<
  {
    arguments: arguments;
    funcs: {
      $__wasm_call_ctors: $__wasm_call_ctors;
      $countAliveNeighbors: $countAliveNeighbors;
      $updateGrid: $updateGrid;
      $entry: $entry;
      $_initialize: $_initialize;
      $__memcpy: $__memcpy;
      $memset: $memset;
      $stackSave: $stackSave;
      $stackRestore: $stackRestore;
      $stackAlloc: $stackAlloc;
      $emscripten_stack_init: $emscripten_stack_init;
      $emscripten_stack_get_free: $emscripten_stack_get_free;
      $emscripten_stack_get_base: $emscripten_stack_get_base;
      $emscripten_stack_get_end: $emscripten_stack_get_end;
      $__errno_location: $__errno_location;
    };
    globals: {
      $__stack_pointer: 5243920;
      $__stack_end: 0;
      $__stack_base: 0;
    };
    memory: {};
    memorySize: 256;
    indirect: ['$__wasm_call_ctors'];
  },
  debugMode
>
